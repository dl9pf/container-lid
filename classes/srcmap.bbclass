...
python do_srcmap_patch () {
    ...
    src_url = (d.getVar('SRC_URI', True) or "").split()
    sourcedir = d.getVar('S', True)
    workdir = d.getVar('WORKDIR', True)

    # Only scan the patched files
    (pkgfile_list, depurl_list, patch_list) = segment_urls(d, src_url)
...
python do_srcmap_unpack () {
    ...
    depfile_list = process_dependencies(d, depurl_list, workdir, sourcedir)
    ...
    for url in src_url:
        ...
        if "file://"+local not in [ x.split(";")[0] for x in patches ]:
            if local.startswith(file_dirname):
                pkgfile_list.append((url, local))
            else:
                depurl_list.append((url, local))
        else:
            patch_list.append((url, local))

    #bb.warn("PKGFILE_LIST: " + " ".join([ x[0]+" "+x[1] for x in pkgfile_list ]))
    #bb.warn("PATCH_LIST: " + " ".join([ x[0]+" "+x[1] for x in patch_list ]))
    #bb.warn("DEPURI_LIST: " + " ".join([ x[0]+" "+x[1] for x in depurl_list ]))

    return (pkgfile_list, depurl_list, patch_list)

def process_dependencies(d, depurl_list, workdir, sourcedir):
    import json
    import codecs
    ...
    for dep in depurl_list:
        ...
        if os.path.isfile(srcmapfile):
            with codecs.open(srcmapfile, mode='r', encoding='utf-8') as f:
                ...
                if srcmapdoc['URI'] == url:
                    depfile_list.append((url, srcmapfile))
                    continue
        ...
        # If the URI is a directory, then archive it
        elif os.path.isdir(localfile):
            import shutil
            import tarfile

            archiveurl = dname_archived+".tar.gz"
            if not os.path.exists(dname_archived_done):
                tar = tarfile.open(archiveurl, 'w:gz')
                tar.add(localfile, arcname=os.path.basename(localfile))
                tar.close()

            # The location of the extracted source is the original URI
...
def process_package(d, provides_unpacked_src, package_url_list, depfile_list, unresolved_src_dir):
    import json
    import codecs
    ...
    srcmap_doc['PN'] = pn
    srcmap_doc['PV'] = pv
    srcmap_doc['Files'] = package_url_list
...
def process_src_url(d, url_data):
    import json
    import codecs

    bb.warn("Processing : " + url_data[0])
    info = {}
    for f in url_data:
        #bb.warn("URIDATA: " + f)
    ...
    if 'subdir' in info:
        ...
        # TODO find downloaded src location for url
...
do_srcmapall[recrdeptask] = "do_srcmapall do_srcmap_deploy"
